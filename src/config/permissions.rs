...
///
/// Wildcards:
/// - `"*"` in `dm_allowed_users` or `group_allowed_users` means allow all users
...
impl SignalPermissions {
    ...
    fn from_bindings_for_adapter(
        ...
        // Merge dm_allowed_users + group_allowed_users for user filtering
        // - "*" means allow all users
        // - Empty list means block all users
        // - Specific list means only those users are allowed
        let mut all_allowed_users = seed_dm_allowed_users.clone();

        // Check for wildcard in seed
        if all_allowed_users.iter().any(|id| id.trim() == "*") {
            return Self {
                group_filter,
                dm_allowed_users: vec!["*".to_string()],
            };
        ...
        // Add seed group_allowed_users from config/instance
        for id in &seed_group_allowed_users {
            let id = id.trim().to_string();
            if id.is_empty() {
                continue;
            }
            ...
            if !all_allowed_users.contains(&id) {
                all_allowed_users.push(id);
            }
        }
        ...
        Self {
            group_filter,
            dm_allowed_users: all_allowed_users,
        }
    }
}
