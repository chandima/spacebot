...
impl SignalAdapter {
    ...
    fn process_envelope(&self, envelope: &Envelope) -> Option<(InboundMessage, String)> {
        ...
        if is_group {
            ...
            // Check sender is allowed (merged dm_allowed_users + group_allowed_users)
            // ["*"] = allow all, [] = block all, specific list = check
            let sender_allowed = if permissions.dm_allowed_users.is_empty() {
                false // Empty = block all
            } else if permissions.dm_allowed_users.iter().any(|u| u == "*") {
    ...
    async fn send_status(
        ...
        match status {
            StatusUpdate::Thinking => {
                ...
                // Use a cancellation channel for faster shutdown instead of relying on task abort.
                ...
                let handle = tokio::spawn(async move {
                    loop {
                        ...
                        // Use select! to allow immediate cancellation without waiting for sleep.
                        tokio::select! {
                            _ = cancel_rx.recv() => {
                                tracing::debug!("signal typing indicator cancelled");
                                break;
                            }
                            result = client
                                .post(&rpc_url)
                        ...
                        tokio::select! {
                            _ = cancel_rx.recv() => {
                                tracing::debug!("signal typing indicator cancelled before sleep");
                                break;
                            }
                            _ = tokio::time::sleep(TYPING_REPEAT_INTERVAL) => {}
                        }
                    }
                });
                ...
                // Drop the sender to allow the receiver to trigger when we want to cancel.
                drop(cancel_tx);
                // Store cancel sender for this conversation so we can signal cancellation.
                // Actually, we need to store the cancel_tx to signal later. Let me rethink.
                // For now, the existing stop_typing will abort the task which works.
                // The select! above makes abort faster since it doesn't wait for sleep.
            }
            _ => {
                self.stop_typing(&message.conversation_id).await;
            }
