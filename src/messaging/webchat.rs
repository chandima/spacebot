...
impl Messaging for WebChatAdapter {
    ...
    async fn broadcast(&self, target: &str, response: OutboundResponse) -> crate::Result<()> {
        let text = extract_webchat_broadcast_text(response)?;
        ...
        .map_err(|_| {
            crate::messaging::traits::mark_retryable_broadcast(anyhow::anyhow!(
                "webchat broadcast dropped: no active event subscribers"
            ))
        })?;
...
fn extract_webchat_broadcast_text(response: OutboundResponse) -> crate::Result<String> {
    match response {
        ...
        other => {
            Err(crate::messaging::traits::unsupported_broadcast_variant_error("webchat", &other))
        }
    }
}
