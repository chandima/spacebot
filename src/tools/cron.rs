...
impl CronTool {
    async fn create(&self, args: CronArgs) -> Result<CronOutput, CronError> {
        ...
        if let Some(expr) = cron_expr.as_deref() {
            ...
            // with 1-based DOW (1=Sun). Standard cron uses 0-based (0=Sun).
            // expand_cron_expr handles both the field expansion and DOW translation.
            let expanded = crate::cron::scheduler::expand_cron_expr(expr);
